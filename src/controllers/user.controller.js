...
const generateAccessAndRefreshTokens = async (userId) => {
  try {
    const user = await User.findNyID(userId);
...
const registerUser = asyncHandler(async (req, res) => {
  ...
  //check is user already present: username,email
  ...
  if (
    req.files &&
    Array.isArray(req.files.console) &&
...
const changeCurrentPassword = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, {}, "Password Channed successfully"));
});
...
const updateUserAvatar = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(newApiResponse(200, user, "Avatar Image updated successfully"));
});
